...
def control_job(job_id: int, target_status: str, reason: str | None = None):
...
def control_stage(job_id: int, phase_code: str, target_state: str, reason: str | None = None):
    job = db.get_job_by_id(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def control_step(image_id: int, phase_code: str, target_status: str, reason: str | None = None):
    try:
