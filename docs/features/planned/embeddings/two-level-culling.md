...
1. Split images into **sub-stacks** via one embedding pass at `level2` (OpenCLIP L/14 by default).
...
```mermaid
flowchart TD
  imgs[Folder images] --> s1["Step 1: root stacks Level 1 MobileNet"]
  s1 --> jit["JIT level2 embed stacked images if missing"]
  jit --> s2["Step 2: sub-clusters level2 OpenCLIP @ threshold"]
  s2 --> leaves[Leaf sub-stacks persisted]
...
| `culling.two_level.level2.embedding_space` | `openclip_l14_laion2b_image` | Sub-stacking space (JIT-generated when enabled — see below) |
...
### Fallback when `enabled=true`

Two-level runs **only** for root stacks with `stack_id IS NOT NULL` and **≥2 images**
([`selection.py`](../../../../modules/selection.py)). Other groups use the legacy path:

| Group | Path | Result |
|-------|------|--------|
| Unstacked bucket (`stack_id IS NULL`, all images in folder) | Legacy 33/33 on whole bucket | ~floor(n×0.33) pick/reject |
| Singleton root stack (1 image) | `classify_sorted_ids` n=1 | neutral |
| Multi-image root stack | `process_stack_two_level` | best-M / N-cap per sub-stack |

Diagnostic SQL: [`05_cull_decision_distribution.sql`](../../../../scripts/sql/culling_analytics_diagnostics/05_cull_decision_distribution.sql).
...
>
> **Level-2 vectors (JIT):** When `culling.two_level.enabled` is true,
> [`SelectionService`](../../../../modules/selection.py) calls
> [`ensure_embeddings_for_space`](../../../../modules/culling_embeddings.py) after
> clustering and before pick assignment. Scope: **images in multi-image stacks only**
> (unstacked and singleton stacks skip level-2 inference). Uses
> `CullingEmbedder` + `generate_and_persist` for the configured `level2.embedding_space`
> when it is a supported culling tower. Unreadable files still fall back to a single
> sub-stack bucket via `compute_sub_clusters`.
>
> **Bulk backfill (optional):** For existing libraries before the first enabled run, or
> to embed all images (not just stacked), use
> `python scripts/maintenance/backfill_culling_embeddings.py --space openclip_l14_laion2b_image`.
> After initial coverage, new stacked images are embedded automatically on the next
> culling run per folder.
>
...
2. **Generate** the vectors. With `culling.two_level.enabled: true`, stacked images
   are embedded automatically on each culling run via `modules/culling_embeddings.py`.
   For bulk/offline coverage (whole library or non-stacked images), add the code to
   `embeddings.culling_spaces` and backfill (WSL, `~/.venvs/tf`):
   ```bash
   python scripts/maintenance/backfill_culling_embeddings.py --space openclip_l14_laion2b_image
   ```
...
| [`modules/culling_embeddings.py`](../../../../modules/culling_embeddings.py) | JIT `ensure_embeddings_for_space` (stack-scoped) |
...
- Unit: `tests/test_two_level_culling.py`, `tests/test_culling_embeddings.py`, extend `tests/test_sub_clustering.py`.
...
library **without** re-clustering, run `scripts/maintenance/backfill_sub_stacks.py` (idempotent,
...
For a full **re-cluster** (root stacks), see
[Step 9 — library-wide CLIP re-cluster rollout](../../../guides/CULLING_EMBEDDING_BACKFILL.md#step-9--library-wide-clip-re-cluster-rollout-optional).

```bash
python scripts/maintenance/backfill_sub_stacks.py --dry-run --limit 10   # smoke
python -u scripts/maintenance/backfill_sub_stacks.py                     # whole library
```
