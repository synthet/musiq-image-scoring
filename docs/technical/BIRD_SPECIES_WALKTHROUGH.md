...
- **A missing box is phase work.** `bird_bbox` is a product of this phase, so `get_phase_incomplete_sql("bird_species")` and `is_image_bird_species_complete` both treat a bird-tagged image as incomplete when `bird_bbox IS NULL` **or** it holds a retryable failure sentinel (`modules/bird_detection.RETRYABLE_BBOX_ERRORS`, currently just `detector_unavailable`). Such folders surface as `awaiting_bird_species` and **Drive to Complete** repairs them on its own. Data-terminal sentinels (`decode_error`, `file_missing`, `detect_error`, `classify_error`) are deliberately *not* retryable — re-queueing them would never converge.
