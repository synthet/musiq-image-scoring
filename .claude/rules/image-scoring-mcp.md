...
|--------|------|--------------|
| **`is-be-*`** | backend | **`is-be-mcp`**, optional **`is-be-webui`** |
...
| **`is-be-webui`** | SSE | **Default compact** — same `search`, `dispatch` as `is-be-mcp`; **`MCP_SSE_PROFILE=full`** for legacy tools + `execute_code` |
...
- **Gradio in-process debug** → **`is-be-webui`** + `execute_code` (`ENABLE_MCP_EXECUTE_CODE=1`)
...
**SSE:** **`is-be-webui`** attaches to the live WebUI; `execute_code` runs in that process when enabled.
