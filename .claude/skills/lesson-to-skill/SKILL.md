...
- You hit an environment quirk (WSL, CUDA, Postgres, MCP) and found a workaround.
...
2. It is stable, not a one-time glitch (a flaky GPU job or a transient DB lock is not a lesson).
...
Default to enriching. Many capabilities here live under local names (`agent-search`, `agent-cli-hub`,
`agent-dev-tooling`, `wsl-environment`, `graphify`) — check
